...
class DemoProvider(BaseLLMProvider):
    def generate(self, system_prompt: str, user_prompt: str, max_tokens: int = 2000) -> str:
        ...
        if "evaluate" in prompt_lower or "score" in prompt_lower or "evaluation" in system_lower:
            # If the user is already answering a follow-up, do not return another follow-up!
            follow_up = ""
            if "follow-up" not in prompt_lower and "follow_up" not in prompt_lower:
                follow_up = "That makes sense. Can you explain why thread synchronization is necessary and mention one common mechanism to achieve it?"
